...
def get_explanation_prompt(answer_type):
    """Get explanation prompt based on answer type (unchanged)"""
...
# ---- New: verbalization prompt for composite relations ----
composite_verbalize_prompt = ChatPromptTemplate.from_template("""
You are an expert educational writer. Given the following structured SUMMARIA relations (Evidence, Emphasis, Contrast) and simple metrics,
produce a short, human-friendly paragraph for each relation that:
- uses rhetorical connectors (e.g., "since", "but", "especially")
- includes the most important metric(s)
- is concise (1-2 sentences per relation)

Input:
{relations}

Produce the paragraphs under headers: Evidence, Emphasis, Contrast.
""")
